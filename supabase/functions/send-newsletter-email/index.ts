...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Check rate limit
    const isAllowed = await checkRateLimit(supabase, ipAddress, "send-newsletter-email");
    if (!isAllowed) {
    ...
    console.log("Sending newsletter signup notification:", { email });
    ...
    const emailResponse = await resend.emails.send({
      ...
      html: `
        ...
        <hr>
        <p><em>This subscriber wants to receive updates on events, programs, and the Social Factory launch.</em></p>
      `,
    });
